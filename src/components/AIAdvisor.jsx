...
export default function AIAdvisor() {
  ...
  const res = await fetch('/api/ai/query', {
      ...
      const data = await res.json();
      const botMsg = { role: 'assistant', text: data.answer || 'No answer returned' };
      ...
      {model && (
        <div style={{ marginTop: 12 }} className="card">
          ...
          {optResult && (
            <div style={{ marginTop: 12 }}>
              <h4>Allocation</h4>
              <pre>{JSON.stringify(optResult.allocation, null, 2)}</pre>
              <h4>Estimated Profit</h4>
              <div>${Math.round(optResult.totalProfit).toLocaleString()}</div>
              <h4>AI Explanation</h4>
              <div style={{ whiteSpace: 'pre-wrap' }}>{optResult.explanation}</div>
            </div>
          )}
        </div>
      )}
